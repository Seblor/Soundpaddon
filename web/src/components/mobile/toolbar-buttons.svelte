<script lang="ts">
  ...
  import { isAudioPlaying, stopAudio, togglePause } from "$lib/demo/demo-audio";
...
<div class="fixed left-2 gap-2 flex">
  <button
    type="button"
  ...
  <button
    ...
    class="btn variant-filled btn-icon variant-filled-primary"
    disabled={(checkIsDemo() ? $isAudioPlaying : isSoundPlaying) === false}
    on:click={() => (checkIsDemo() ? togglePause() : soundpadClient.togglePause())}
  >
    <span><PauseIcon /></span>
  </button>
</div>
...
<div class="fixed left-1/2 -translate-x-1/2">
  <div
    ...
    class="flex gap-2 {checkIsDemo() ? '[&>*]:pointer-events-none' : ''}"
